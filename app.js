...
function bindEvents(){
  ...
  $('rosterSeason').addEventListener('change',()=>{state.rosterSelected={};state.rosterSlotSeasons={};syncRosterTeamList();buildLineupEditor();renderRosterCard()});$('rosterTeam').addEventListener('change',()=>{buildLineupEditor();renderRosterCard()});$('rosterMinGP').addEventListener('input',()=>{buildLineupEditor();renderRosterCard()});$('clearRosterBtn').addEventListener('click',()=>{state.rosterSelected={};state.rosterSlotSeasons={};buildLineupEditor();renderRosterCard()});$('autoTeamBtn').addEventListener('click',autoLoadTeam);$('autoBestBtn').addEventListener('click',autoBest);$('resetRoleBtn').addEventListener('click',()=>{state.roleConfig=defaultRoleConfig();buildRoleSettings();buildLineupEditor();renderRosterCard()});
...
function autoBest(){state.rosterSelected={};state.rosterSlotSeasons={};const used=new Set();for(const role of roles){const cand=rosterCandidates(role).filter(r=>!used.has(r.player));for(let i=0;i<role.count;i++){const r=cand[i];if(r){state.rosterSelected[slotId(role.key,i)]=r.key;used.add(r.player)}}}buildLineupEditor();renderRosterCard()}
