...
async function init(){
  try{
    ...
    const coreResponses=await Promise.all(state.fullManifest.seasons.map(s=>fetch(`data/core/${s}.csv`,{cache:'no-store'})));
    const bad=coreResponses.find(r=>!r.ok); if(bad)throw new Error(`Core season failed: ${bad.status}`);
...
function bindEvents(){
  ...
  $('fullStatsBtn').addEventListener('click',openFullStats);$('closeFullStatsBtn').addEventListener('click',()=> $('fullStatsPanel').classList.add('hidden'));
...
function renderPlayer(){
  ...
  $('fullStatsPanel').classList.add('hidden');renderTimeline();
}
...
async function loadFullSeason(season){if(state.fullSeasonCache.has(season))return state.fullSeasonCache.get(season);const resp=await fetch(`data/${season}.csv`,{cache:'no-store'});if(!resp.ok)throw new Error(`Season ${season} failed`);const rows=csvObjects(await resp.text());state.fullSeasonCache.set(season,rows);return rows}
const statGroups={
  'Individual':['season','name','team','position','games_played','icetime','penalties','I_F_penalityMinutes','I_F_faceOffsWon','I_F_hits','I_F_takeaways','I_F_giveaways','shotsBlockedByPlayer','faceoffsWon','faceoffsLost','timeOnBench','penalityMinutes','penalityMinutesDrawn','penaltiesDrawn'],
  'Individual Shooting':['I_F_lowDangerShots','I_F_mediumDangerShots','I_F_highDangerShots','I_F_lowDangerxGoals','I_F_mediumDangerxGoals','I_F_highDangerxGoals','I_F_lowDangerGoals','I_F_mediumDangerGoals','I_F_highDangerGoals','I_F_scoreAdjustedShotsAttempts','I_F_unblockedShotAttempts','I_F_scoreAdjustedUnblockedShotAttempts','I_F_xGoalsFromxReboundsOfShots','I_F_xGoalsFromActualReboundsOfShots','I_F_reboundxGoals','I_F_xGoals_with_earned_rebounds','I_F_xGoals_with_earned_rebounds_scoreAdjusted','I_F_xGoals_with_earned_rebounds_scoreFlurryAdjusted'],
  'Shifts':['I_F_shifts','I_F_oZoneShiftStarts','I_F_dZoneShiftStarts','I_F_neutralZoneShiftStarts','I_F_flyShiftStarts','I_F_oZoneShiftEnds','I_F_dZoneShiftEnds','I_F_neutralZoneShiftEnds','I_F_flyShiftEnds'],
  'On-Ice For':[], 'On-Ice Against':[], 'Off-Ice / After Shift':[]
};
function groupColumns(headers){const g=JSON.parse(JSON.stringify(statGroups));g['On-Ice For']=headers.filter(h=>h.startsWith('OnIce_F_'));g['On-Ice Against']=headers.filter(h=>h.startsWith('OnIce_A_'));g['Off-Ice / After Shift']=headers.filter(h=>h.startsWith('OffIce_')||/AfterShifts$/.test(h));return g}
function prettyStat(s){return s.replace(/^I_F_/,'').replace(/^OnIce_F_/,'').replace(/^OnIce_A_/,'').replace(/^OffIce_/,'Off-Ice ').replace(/_/g,' ').replace(/([a-z])([A-Z])/g,'$1 $2').replace(/\bx Goals\b/gi,'xGoals').replace(/\bI F\b/gi,'').replace(/\bpenality\b/gi,'Penalty').replace(/\b([a-z])/g,m=>m.toUpperCase())}
async function openFullStats(){
  const r=currentPlayerRow();if(!r)return;$('fullStatsPanel').classList.remove('hidden');$('fullStatsTitle').textContent=`${r.player} • ${displaySeason(r.season)}`;$('fullStatsGrid').innerHTML='<div class="full-stat-item"><span>Loading full workbook row…</span><strong>…</strong></div>';
  try{const rows=await loadFullSeason(r.season);const matches=rows.filter(x=>clean(x.player).toLowerCase()===r.player.toLowerCase());if(!matches.length)throw new Error('Player row not found');const headers=Object.keys(matches[0]);const grouped=groupColumns(headers);const agg={};for(const h of headers){if(['player','position_group','magpie_score','avgtoi'].includes(h))continue;const values=matches.map(x=>x[h]).filter(v=>clean(v)!=='');if(!values.length){agg[h]='';continue}const allNumeric=values.every(v=>Number.isFinite(Number(v)));agg[h]=allNumeric?values.reduce((s,v)=>s+Number(v),0):[...new Set(values)].join(', ')}agg.player=r.player;agg.avgtoi=r.avgtoi;agg.magpie_score=r.score;
    const tabs=$('fullStatsTabs');tabs.innerHTML=Object.keys(grouped).map((g,i)=>`<button class="stat-tab ${i===0?'active':''}" data-group="${esc(g)}">${esc(g)}</button>`).join('');const renderGroup=g=>{$('fullStatsGrid').innerHTML=(grouped[g]||[]).filter(h=>h in agg).map(h=>`<div class="full-stat-item"><span>${esc(prettyStat(h))}</span><strong>${formatRawStat(h,agg[h])}</strong></div>`).join('')||'<div class="full-stat-item"><span>No values</span><strong>—</strong></div>';tabs.querySelectorAll('.stat-tab').forEach(b=>b.classList.toggle('active',b.dataset.group===g))};tabs.querySelectorAll('.stat-tab').forEach(b=>b.addEventListener('click',()=>renderGroup(b.dataset.group)));renderGroup(Object.keys(grouped)[0]);$('fullStatsPanel').scrollIntoView({behavior:'smooth',block:'start'});
  }catch(e){console.error(e);$('fullStatsGrid').innerHTML='<div class="full-stat-item"><span>Full stats could not be loaded.</span><strong>—</strong></div>'}
}
function formatRawStat(h,v){if(clean(v)==='')return'—';if(!Number.isFinite(Number(v)))return esc(v);if(h==='icetime'||h==='timeOnBench')return `${fmt(num(v)/60,1)} min`;const n=Number(v);return Math.abs(n-Math.round(n))<1e-8?whole(n):fmt(n,2)}
...
function renderCompare(){const a=compareRow('A'),b=compareRow('B');if(!a||!b)return;$('compareNameA').textContent=`${a.player} • ${displaySeason(a.season)}`;$('compareNameB').textContent=`${b.player} • ${displaySeason(b.season)}`;const metrics=[['Games Played','gp',false,0],['Magpie Score','score',true,2],['Hits','hits',true,0],['Blocked Shots','blocked',true,0],['Takeaways','takeaways',true,0],['Goals For','goalsFor',true,0],['Shots For','shotsFor',true,0],['Goals Against','goalsAgainst',false,0],['Shots Against','shotsAgainst',false,0],['Avg TOI','avgtoi',true,1]];$('comparisonBody').innerHTML=metrics.map(([label,key,higher,d])=>{const av=num(a[key]),bv=num(b[key]),leadA=higher?av>bv:av<bv,leadB=higher?bv>av:bv<av;const pa=key==='gp'?'':`${Math.round(percentile(a,key))}th pct`;const pb=key==='gp'?'':`${Math.round(percentile(b,key))}th pct`;return `<div class="compare-row"><div class="compare-value ${leadA?'lead':''}">${fmt(av,d)} <span class="percentile">${pa}</span></div><div class="compare-label">${esc(label)}</div><div class="compare-value right ${leadB?'lead':''}"><span class="percentile">${pb}</span> ${fmt(bv,d)}</div></div>`}).join('')}

function syncTeamList(){const s=$('teamSeason').value,teams=state.teamsBySeason.get(s)||[];setOptions($('teamSelect'),teams.map(opt).join(''),teams[0]);}
...
function renderTeamCard(){const rows=teamContextRows(true);const s=$('teamSeason').value,t=$('teamSelect').value;$('teamCardSeason').textContent=displaySeason(s);$('teamCardName').textContent=t||'Select a team';const notes=[];if(state.teamAddedKey)notes.push('1 simulated addition');if(state.teamRemoved.size)notes.push(`${state.teamRemoved.size} removed`);$('teamCardSub').textContent=`Minimum ${whole(num($('teamMinGP').value))} GP${notes.length?' • '+notes.join(' • '):''}`;$('teamAvgScore').textContent=rows.length?fmt(average(rows.map(r=>r.score))):'—';renderTeamHighlights();renderTeamTable();renderTeamAdjusters()}
...
function renderTeamTable(){const rows=teamContextRows(true).slice().sort((a,b)=>b.score-a.score||b.gp-a.gp);const hi=new Set([$ ('teamHighlightA').value,$('teamHighlightB').value].filter(Boolean));$('teamTable').innerHTML=rows.map((r,i)=>`<div class="team-row ${hi.has(r.key)?'highlight':''} ${r.simAdded?'added':''}"><span class="rank">${i+1}</span><span class="player-name">${esc(r.player)}${r.simAdded?'<b class="tag">Added</b>':''}</span><span>${r.position}</span><span>${whole(r.gp)}</span><span>${whole(r.hits)}</span><span>${whole(r.blocked)}</span><span>${whole(r.takeaways)}</span><span>${whole(r.goalsFor)}</span><span>${whole(r.shotsFor)}</span><span class="score">${fmt(r.score)}</span></div>`).join('')||'<div class="team-row"><span></span><span>No players match the filters.</span></div>'}
...
function defaultRoleConfig(){return {f1:{min:18,max:Infinity,toi:21},f2:{min:15,max:18,toi:16.5},f3:{min:12,max:15,toi:13.5},f4:{min:1,max:12,toi:6.5},d1:{min:22,max:Infinity,toi:25},d2:{min:17,max:22,toi:19.5},d3:{min:0,max:17,toi:8.5}}}
...
function syncRosterTeamList(){const s=$('rosterSeason').value,teams=state.teamsBySeason.get(s)||[];const prev=$('rosterTeam').value;setOptions($('rosterTeam'),'<option value="">All teams</option>'+teams.map(opt).join(''),teams.includes(prev)?prev:'')}
function slotId(roleKey,i){return `${roleKey}-${i}`}
function rosterCandidates(role){const s=$('rosterSeason').value,t=$('rosterTeam').value,minGP=num($('rosterMinGP').value),c=state.roleConfig[role.key];return state.rows.filter(r=>r.season===s&&r.position===role.position&&r.gp>=minGP&&(!t||r.team.split(',').map(clean).includes(t))&&r.avgtoi>=c.min&&r.avgtoi<=c.max).sort((a,b)=>b.score-a.score||b.avgtoi-a.avgtoi)}
function buildLineupEditor(){if(!state.roleConfig)state.roleConfig=defaultRoleConfig();$('lineupEditor').innerHTML=roles.map(role=>{const cand=rosterCandidates(role);const slots=Array.from({length:role.count},(_,i)=>{const id=slotId(role.key,i),cur=state.rosterSelected[id]||'';if(cur&&!cand.some(r=>r.key===cur))state.rosterSelected[id]='';return `<div class="slot"><label>${role.position==='F'?'Forward':'Defence'} ${i+1}<select data-slot="${id}" data-role="${role.key}"><option value="">Select player</option>${cand.map(r=>`<option value="${esc(r.key)}" ${r.key===cur?'selected':''}>${esc(r.player)} • ${fmt(r.avgtoi,1)} TOI • ${fmt(r.score)}</option>`).join('')}</select></label><div class="slot-meta">${cand.length} eligible • ${fmt(state.roleConfig[role.key].toi,1)} assigned TOI</div></div>`}).join('');return `<div class="line-group"><div class="line-group-title"><strong>${role.label}</strong><span>${state.roleConfig[role.key].min}–${Number.isFinite(state.roleConfig[role.key].max)?state.roleConfig[role.key].max:'∞'} pool TOI</span></div><div class="slot-grid ${role.position==='D'?'defence':''}">${slots}</div></div>`}).join('');$('lineupEditor').querySelectorAll('select[data-slot]').forEach(s=>s.addEventListener('change',()=>{state.rosterSelected[s.dataset.slot]=s.value;enforceUnique(s.dataset.slot,s.value);buildLineupEditor();renderRosterCard()}));updateSelectedCount()}
function enforceUnique(changed,key){if(!key)return;for(const [slot,v] of Object.entries(state.rosterSelected))if(slot!==changed&&v===key)state.rosterSelected[slot]=''}
...
function fullGameScale(entries){const assigned=entries.reduce((s,e)=>s+state.roleConfig[e.role.key].toi,0),target=300*(entries.length/18);return assigned>0?target/assigned:1}
function projectedPlayer(e,scale){const r=e.row,gp=r.gp||1,actual=r.avgtoi||1,assigned=state.roleConfig[e.role.key].toi*scale,k=assigned/actual;return {assigned,hits:(r.hits/gp)*k,blocked:(r.blocked/gp)*k,takeaways:(r.takeaways/gp)*k,minors:(r.minors/gp)*k,ga:(r.goalsAgainst/gp)*k,sa:(r.shotsAgainst/gp)*k,gf:(r.goalsFor/gp)*k,sf:(r.shotsFor/gp)*k}}
function rosterTotals(){const entries=getSelectedEntries(),scale=fullGameScale(entries),t={entries,scale,hits:0,blocked:0,takeaways:0,minors:0,ga:0,sa:0,gf:0,sf:0,assigned:0};entries.forEach(e=>{const p=projectedPlayer(e,scale);for(const k of ['hits','blocked','takeaways','minors','ga','sa','gf','sf','assigned'])t[k]+=p[k]});const avgAssigned=entries.length?t.assigned/entries.length:0;t.score=magpie({hits:t.hits,blocked:t.blocked,takeaways:t.takeaways,minors:t.minors,goalsAgainst:t.ga,shotsAgainst:t.sa,avgtoi:avgAssigned});return t}
function renderRosterCard(){updateSelectedCount();const t=rosterTotals(),s=$('rosterSeason').value;$('rosterCardSeason').textContent=displaySeason(s);$('rosterCardSub').textContent=`${t.entries.length} of 18 skaters selected${t.entries.length?` • ${fmt(t.scale,3)}× full-game normalization`:''}`;$('rosterScore').textContent=fmt(t.score);$('rosterHits').textContent=fmt(t.hits,1);$('rosterBlocks').textContent=fmt(t.blocked,1);$('rosterTakeaways').textContent=fmt(t.takeaways,1);$('rosterGF').textContent=fmt(t.gf,1);$('rosterSF').textContent=fmt(t.sf,1);$('rosterGA').textContent=fmt(t.ga,1);$('rosterSA').textContent=fmt(t.sa,1);$('rosterCardLines').innerHTML=roles.map(role=>{const players=Array.from({length:role.count},(_,i)=>{const row=state.byKey.get(state.rosterSelected[slotId(role.key,i)]);return row?`<div class="roster-player"><strong>${esc(row.player)}</strong><small>${esc(row.team)} • ${fmt(row.score)} Magpie</small></div>`:'<div class="roster-player"><strong>Open</strong><small>Select player</small></div>'}).join('');return `<div class="roster-line"><div class="roster-line-label">${esc(role.position==='F'?role.label.replace(' Forwards',''):role.label)}</div><div class="roster-players ${role.position==='D'?'defence':''}">${players}</div></div>`}).join('')}
function autoLoadTeam(){const team=$('rosterTeam').value;if(!team){alert('Choose a team first.');return}const s=$('rosterSeason').value,minGP=num($('rosterMinGP').value),rows=state.rows.filter(r=>r.season===s&&r.gp>=minGP&&r.team.split(',').map(clean).includes(team));const f=rows.filter(r=>r.position==='F').sort((a,b)=>b.avgtoi-a.avgtoi||b.score-a.score).slice(0,12),d=rows.filter(r=>r.position==='D').sort((a,b)=>b.avgtoi-a.avgtoi||b.score-a.score).slice(0,6);state.rosterSelected={};let fi=0,di=0;for(const role of roles){for(let i=0;i<role.count;i++){const r=role.position==='F'?f[fi++]:d[di++];if(r)state.rosterSelected[slotId(role.key,i)]=r.key}}buildLineupEditor();renderRosterCard()}
function autoBest(){state.rosterSelected={};const used=new Set();for(const role of roles){const cand=rosterCandidates(role).filter(r=>!used.has(r.key));for(let i=0;i<role.count;i++){const r=cand[i];if(r){state.rosterSelected[slotId(role.key,i)]=r.key;used.add(r.key)}}}buildLineupEditor();renderRosterCard()}
